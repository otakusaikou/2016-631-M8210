...
def getPoint3d(ptFileName, IO, EO):
    ...
    objPts = np.genfromtxt(ptFileName, dtype=[
        ('X', 'f8'), ('Y', 'f8'), ('Z', 'f8')], skip_header=1, delimiter=';')
...
def main(IOFileName, EOFileName, imgFileName, ptFileName, outputPtFileName):
    IO = getIO(IOFileName)
...
if __name__ == '__main__':
    # Define file names
    import glob
    EOFileList = glob.glob('./param/EO*')
    imgFileList = map(lambda x: x.replace("param/EO_", "images/").replace("txt", "jpg"), EOFileList)
    outputPtFileList = map(lambda x: x.replace("param/EO_", "ptCloud/stable/").replace("txt", "txt"), EOFileList)
    IOFileName = './param/IO.txt'
    ptFileName = './ptCloud/stable.txt'

    for i in range(len(EOFileList)):
        EOFileName = EOFileList[i]
        outputPtFileName = outputPtFileList[i]
        imgFileName = imgFileList[i]
        print imgFileName
        main(IOFileName, EOFileName, imgFileName, ptFileName, outputPtFileName)
